...
RUN npm install -g "node-gyp@${NODE_GYP_VERSION}"     && npm install -g "nyuu@${NYUU_VERSION}" --unsafe-perm
...
RUN git clone --branch "${PAR2_TAG}" --depth=1 https://github.com/animetosho/par2cmdline-turbo.git /tmp/par2cmdline-turbo     && cd /tmp/par2cmdline-turbo     && (./automake.sh || autoreconf -fi)     && ./configure --prefix=/usr/local     && make -j"$(nproc)"     && make install     && strip /usr/local/bin/par2 || true     && rm -rf /tmp/par2cmdline-turbo


FROM python:3.13.12-slim-bookworm AS cython-builder

ENV PYTHONDONTWRITEBYTECODE=1
ENV PYTHONUNBUFFERED=1
ENV PIP_PREFER_BINARY=1
ENV PREPAC_LOG_LEVEL=INFO
ENV PREPAC_LOG_JSON=false
...
RUN pip install --upgrade pip setuptools wheel     && pip install --no-cache-dir -r requirements.txt     && pip install --no-cache-dir Cython

COPY app ./app
COPY setup.py ./setup.py
RUN python setup.py build_ext --inplace     && find app -type f -name '*.c' -delete     && find app -type f -name '*.py' ! -name '__init__.py' ! -name 'name_randomizer_data.py' ! -name 'version.py' -delete
...
RUN curl -L --retry 3 --retry-delay 2 "${RAR_URL}" -o /tmp/rarlinux.tar.gz     && echo "${RAR_SHA256}  /tmp/rarlinux.tar.gz" | sha256sum -c -     && cd /tmp     && tar -xzf /tmp/rarlinux.tar.gz     && cp /tmp/rar/rar /usr/local/bin/rar     && chmod +x /usr/local/bin/rar     && strip /usr/local/bin/rar || true     && rm -rf /tmp/rar /tmp/rarlinux.tar.gz
